...
st.set_page_config(
    layout="wide",
    page_title="لوحة تحليل جودة المياه والطحالب",
    page_icon="🌊"
)
...
with st.sidebar:
    ...
    gamma = st.slider(
        "غاما (القيمة الحالية: %.2f)" % st.session_state.get('gamma', 1.0),
        min_value=0.2, max_value=3.0,
        value=st.session_state.get('gamma', 1.0),
        step=0.1, format="%.2f",
        key='gamma_slider'
    )
    st.session_state.gamma = gamma      # تخزين القيمة
...
indicator_display_names = {
    ...
    "Cyanobacteria (10³ cells/ml)": "البكتيريا الزرقاء (آلاف خلية/مل)",
    "Turbidity (NTU)": "العكارة (NTU)",
    "CDOM (mg/l)": "المادة العضوية الملونة (ملجم/لتر)",
    "DOC (mg/l)": "الكربون العضوي المذاب (ملجم/لتر)",
